...
class JigraGoogleMap(
        val id: String,
        val config: GoogleMapConfig,
        val delegate: JigraGoogleMapsPlugin
) :
    ...
    private val markers = HashMap<String, JigraGoogleMapMarker>()
    private val polygons = HashMap<String, JigraGoogleMapsPolygon>()
    private val circles = HashMap<String, JigraGoogleMapsCircle>()
    private val polylines = HashMap<String, JigraGoogleMapPolyline>()
    private val markerIcons = HashMap<String, Bitmap>()
    private var clusterManager: ClusterManager<JigraGoogleMapMarker>? = null
        ...
        runBlocking {
            val job =
                    CoroutineScope(Dispatchers.Main).launch {
                        ...
                        mapView.onStart()
                        mapView.getMapAsync(this@JigraGoogleMap)
    ...
    fun addMarkers(
            newMarkers: List<JigraGoogleMapMarker>,
                ...
                newMarkers.forEach {
                    val markerOptions: Deferred<MarkerOptions> =
                            CoroutineScope(Dispatchers.IO).async {
                                this@JigraGoogleMap.buildMarker(it)
                            }
    ...
    fun addMarker(marker: JigraGoogleMapMarker, callback: (result: Result<String>) -> Unit) {
        try {
            googleMap ?: throw GoogleMapNotAvailable()
            ...
            CoroutineScope(Dispatchers.Main).launch {
                val markerOptions: Deferred<MarkerOptions> =
                        CoroutineScope(Dispatchers.IO).async {
                            this@JigraGoogleMap.buildMarker(marker)
                        }
                val googleMapMarker = googleMap?.addMarker(markerOptions.await())
    ...
    fun addPolygons(newPolygons: List<JigraGoogleMapsPolygon>, callback: (ids: Result<List<String>>) -> Unit) {
        try {
            googleMap ?: throw GoogleMapNotAvailable()
            ...
            CoroutineScope(Dispatchers.Main).launch {
                newPolygons.forEach {
                    val polygonOptions: Deferred<PolygonOptions> = CoroutineScope(Dispatchers.IO).async {
                        this@JigraGoogleMap.buildPolygon(it)
                    }
    ...
    fun addCircles(newCircles: List<JigraGoogleMapsCircle>,callback: (ids: Result<List<String>>) -> Unit) {
        try {
            googleMap ?: throw GoogleMapNotAvailable()
            ...
            CoroutineScope(Dispatchers.Main).launch {
                newCircles.forEach {
                    var circleOptions: Deferred<CircleOptions> = CoroutineScope(Dispatchers.IO).async {
                        this@JigraGoogleMap.buildCircle(it)
                    }
    ...
    fun addPolylines(newLines: List<JigraGoogleMapPolyline>, callback: (ids: Result<List<String>>) -> Unit) {
        try {
            googleMap ?: throw GoogleMapNotAvailable()
            ...
            CoroutineScope(Dispatchers.Main).launch {
                newLines.forEach {
                    val polylineOptions: Deferred<PolylineOptions> = CoroutineScope(Dispatchers.IO).async {
                        this@JigraGoogleMap.buildPolyline(it)
                    }
    ...
    private fun setClusterManagerRenderer(minClusterSize: Int?) {
        clusterManager?.renderer = JigraClusterManagerRenderer(
    ...
    fun disableClustering(callback: (error: GoogleMapsError?) -> Unit) {
        try {
            ...
            CoroutineScope(Dispatchers.Main).launch {
                ...
                googleMap?.setOnMarkerClickListener(this@JigraGoogleMap)
    ...
    fun removeMarkers(ids: List<String>, callback: (error: GoogleMapsError?) -> Unit) {
        try {
            ...
            CoroutineScope(Dispatchers.Main).launch {
                val deletedMarkers: MutableList<JigraGoogleMapMarker> = mutableListOf()
    ...
    private fun buildCircle(circle: JigraGoogleMapsCircle): CircleOptions {
    ...
    private fun buildPolygon(polygon: JigraGoogleMapsPolygon): PolygonOptions {
    ...
    private fun buildMarker(marker: JigraGoogleMapMarker): MarkerOptions {
    ...
    override fun onMapReady(map: GoogleMap) {
        runBlocking {
            ...
            val data = JSObject()
            data.put("mapId", this@JigraGoogleMap.id)
    ...
    fun setListeners() {
        CoroutineScope(Dispatchers.Main).launch {
            this@JigraGoogleMap.googleMap?.setOnCameraIdleListener(this@JigraGoogleMap)
            this@JigraGoogleMap.googleMap?.setOnCameraMoveStartedListener(
                    this@JigraGoogleMap
            )
            this@JigraGoogleMap.googleMap?.setOnCameraMoveListener(this@JigraGoogleMap)
            this@JigraGoogleMap.googleMap?.setOnMarkerClickListener(this@JigraGoogleMap)
            this@JigraGoogleMap.googleMap?.setOnPolygonClickListener(this@JigraGoogleMap)
            this@JigraGoogleMap.googleMap?.setOnCircleClickListener(this@JigraGoogleMap)
            this@JigraGoogleMap.googleMap?.setOnMarkerDragListener(this@JigraGoogleMap)
            this@JigraGoogleMap.googleMap?.setOnMapClickListener(this@JigraGoogleMap)
            this@JigraGoogleMap.googleMap?.setOnMyLocationButtonClickListener(
                    this@JigraGoogleMap
            )
            this@JigraGoogleMap.googleMap?.setOnMyLocationClickListener(this@JigraGoogleMap)
            this@JigraGoogleMap.googleMap?.setOnInfoWindowClickListener(this@JigraGoogleMap)
            this@JigraGoogleMap.googleMap?.setOnPolylineClickListener(this@JigraGoogleMap)
        }
    }
    ...
    fun setClusterListeners() {
        CoroutineScope(Dispatchers.Main).launch {
            clusterManager?.setOnClusterItemClickListener {
                ...
                else this@JigraGoogleMap.onMarkerClick(it.googleMapMarker!!)
            }

            clusterManager?.setOnClusterItemInfoWindowClickListener {
                if (null != it.googleMapMarker) {
                    this@JigraGoogleMap.onInfoWindowClick(it.googleMapMarker!!)
                }
            }

            clusterManager?.setOnClusterInfoWindowClickListener {
                val data = this@JigraGoogleMap.getClusterData(it)
    ...
    private fun getClusterData(it: Cluster<JigraGoogleMapMarker>): JSObject {
        val data = JSObject()
    ...
    override fun onCameraIdle() {
        ...
        data.put("bearing", this@JigraGoogleMap.googleMap?.cameraPosition?.bearing)
        data.put("latitude", this@JigraGoogleMap.googleMap?.cameraPosition?.target?.latitude)
        data.put("longitude", this@JigraGoogleMap.googleMap?.cameraPosition?.target?.longitude)
        data.put("tilt", this@JigraGoogleMap.googleMap?.cameraPosition?.tilt)
        data.put("zoom", this@JigraGoogleMap.googleMap?.cameraPosition?.zoom)
