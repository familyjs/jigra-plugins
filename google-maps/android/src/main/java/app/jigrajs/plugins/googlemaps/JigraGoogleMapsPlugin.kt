...
@JigraPlugin(
        name = "JigraGoogleMaps",
        permissions =
                [
                        Permission(
                                strings = [Manifest.permission.ACCESS_FINE_LOCATION],
                                alias = JigraGoogleMapsPlugin.LOCATION
                        ),
                ],
)
class JigraGoogleMapsPlugin : Plugin(), OnMapsSdkInitializedCallback {
    ...
    fun create(call: PluginCall) {
        try {
            ...
            val newMap = JigraGoogleMap(id, config, this)
            maps[id] = newMap
    ...
    fun addMarker(call: PluginCall) {
        try {
            ...
            val marker = JigraGoogleMapMarker(markerObj)
    ...
    fun addMarkers(call: PluginCall) {
        try {
            ...
            val markers: MutableList<JigraGoogleMapMarker> = mutableListOf()
    ...
    fun addPolygons(call: PluginCall) {
        try {
            ...
            val polygons: MutableList<JigraGoogleMapsPolygon> = mutableListOf()

            for (i in 0 until polygonsObjectArray.length()) {
                val polygonObj = polygonsObjectArray.getJSONObject(i)
                val polygon = JigraGoogleMapsPolygon(polygonObj)
    ...
    fun addCircles(call: PluginCall) {
        try {
            ...
            val circles: MutableList<JigraGoogleMapsCircle> = mutableListOf()

            for (i in 0 until circlesObjectArray.length()) {
                val circleObj = circlesObjectArray.getJSONObject(i)
                val circle = JigraGoogleMapsCircle(circleObj)
    ...
    @PluginMethod
     fun addPolylines(call: PluginCall) {
         try  {
             ...
             val polylines: MutableList<JigraGoogleMapPolyline> = mutableListOf()

             for (i in 0 until polylinesObjectArray.length()) {
                 val polylineObj = polylinesObjectArray.getJSONObject(i)
                 val polyline = JigraGoogleMapPolyline(polylineObj)
