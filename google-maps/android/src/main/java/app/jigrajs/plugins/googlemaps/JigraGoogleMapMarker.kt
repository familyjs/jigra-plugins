...
class JigraGoogleMapMarker(fromJSONObject: JSONObject): ClusterItem {
    ...
    var iconAnchor: JigraGoogleMapsPoint? = null
    ...
    init {
        ...
        if (fromJSONObject.has("iconAnchor")) {
            val inputAnchorPoint = JigraGoogleMapsPoint(fromJSONObject.getJSONObject("iconAnchor"))
    ...
    private fun buildIconAnchorPoint(iconAnchor: JigraGoogleMapsPoint): JigraGoogleMapsPoint? {
        iconSize ?: return null

        val u: Float = iconAnchor.x / iconSize!!.width
        val v: Float = iconAnchor.y / iconSize!!.height

        return JigraGoogleMapsPoint(u, v)
    }
}
