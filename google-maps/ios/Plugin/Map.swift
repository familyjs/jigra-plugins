...
public class Map {
    ...
    private var delegate: JigraGoogleMapsPlugin

    init(id: String, config: GoogleMapConfig, delegate: JigraGoogleMapsPlugin) {
        self.id = id
        self.config = config
        ...
        DispatchQueue.main.async {
            ...
            if let styles = self.config.styles {
                do {
                    ...
                    JIGLog.print("Invalid Google Maps styles")
                }
            }
        ...
        if let iconUrl = marker.iconUrl {
            if let iconImage = self.markerIcons[iconUrl] {
                ...
                if iconUrl.starts(with: "https:") {
                    ...
                    print("JigraGoogleMaps Warning: could not load image '\(iconUrl)'. \(detailedMessage)  Using default marker icon.")
                }
            }
        } else {
