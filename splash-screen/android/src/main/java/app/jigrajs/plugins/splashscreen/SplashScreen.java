...
public class SplashScreen {
   ...
   */
  private void showWithAndroid12API(
    ...
    activity.runOnUiThread(
      () -> {
        ...
        windowSplashScreen.setKeepOnScreenCondition(
          () -> isVisible || isHiding
        );
    ...
    activity.runOnUiThread(
      () -> {
        if (config.isImmersive()) {
          dialog = new Dialog(activity, R.style.jigra_immersive_style);
        } else if (config.isFullScreen()) {
          dialog = new Dialog(activity, R.style.jigra_full_screen_style);
        } else {
          dialog = new Dialog(activity, R.style.jigra_default_style);
        }
        int splashId = 0;
  ...
  private Drawable getSplashDrawable() {
    ...
    try {
      Drawable drawable = context
        .getResources()
        .getDrawable(splashId, context.getTheme());
      return drawable;
  ...
  private void hide(final int fadeOutDuration, boolean isLaunchSplash) {
    ...
    if (null != this.onPreDrawListener) {
      ...
      if (null != content) {
        content
          .getViewTreeObserver()
          .removeOnPreDrawListener(this.onPreDrawListener);
      }
      this.onPreDrawListener = null;
      return;
    }
