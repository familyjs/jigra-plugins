...
public class SplashScreenPlugin extends Plugin {
  ...
  public void load() {
    ...
    if (
      ...
      bridge.getConfig().getErrorPath() != null &&
      !config.isLaunchAutoHide()
    ) {
      return;
    } else {
  ...
  private SplashScreenConfig getSplashScreenConfig() {
    ...
    Integer duration = getConfig()
      .getInt("launchShowDuration", config.getLaunchShowDuration());
    config.setLaunchShowDuration(duration);
    Integer fadeOutDuration = getConfig()
      .getInt("launchFadeOutDuration", config.getLaunchFadeOutDuration());
    config.setLaunchFadeOutDuration(fadeOutDuration);
    Boolean autohide = getConfig()
      .getBoolean("launchAutoHide", config.isLaunchAutoHide());
    config.setLaunchAutoHide(autohide);
    if (getConfig().getString("androidSplashResourceName") != null) {
      config.setResourceName(
        getConfig().getString("androidSplashResourceName")
      );
    }
    Boolean immersive = getConfig()
      .getBoolean("splashImmersive", config.isImmersive());
    config.setImmersive(immersive);

    Boolean fullScreen = getConfig()
      .getBoolean("splashFullScreen", config.isFullScreen());
    ...
    Boolean showSpinner = getConfig()
      .getBoolean("showSpinner", config.isShowSpinner());
    config.setShowSpinner(showSpinner);

    Boolean useDialog = getConfig()
      .getBoolean("useDialog", config.isUsingDialog());
