...
@objc public class Filesystem: NSObject {
    ...
    public func writeFile(at fileUrl: URL, with data: String, recursive: Bool, with encoding: String?) throws -> String {
        ...
        if encoding != nil {
            ...
            if let base64Data = Data.jigra.data(base64EncodedOrDataUrl: data) {
    ...
    @objc public func downloadFile(call: JIGPluginCall, emitter: @escaping ProgressEmitter, config: InstanceConfiguration?) throws {
        ...
        func handleDownload(downloadLocation: URL?, response: URLResponse?, error: Error?) {
            if let error = error {
                JIGLog.print("Error on download file", String(describing: downloadLocation), String(describing: response), String(describing: error))
            ...
            if let httpResponse = response as? HTTPURLResponse {
                if !(200...299).contains(httpResponse.statusCode) {
                    JIGLog.print("Error downloading file:", urlString, httpResponse)
            ...
            if let foundDir = getDirectory(directory: directory) {
                ...
                do {
                    ...
                    JIGLog.print("Attempting to write to file destination: \(dest.absoluteString)")
                    ...
                    if FileManager.default.fileExists(atPath: dest.relativePath) {
                        do {
                            JIGLog.print("File already exists. Attempting to remove file before writing.")
                    ...
                    JIGLog.print("Downloaded file successfully! \(dest.absoluteString)")
        ...
        let request = try HttpRequestHandler.JigraHttpRequestBuilder()
            .setUrl(urlString)
            .setMethod(method)
            .setUrlParams(params)
            .openConnection()
