...
```xml
<queries>
  <package android:name="com.getjigra.myapp" />
</queries>
```
...
```typescript
import { AppLauncher } from '@jigra/app-launcher';

const checkCanOpenUrl = async () => {
  const { value } = await AppLauncher.canOpenUrl({ url: 'com.getjigra.myapp' });

  console.log('Can open url: ', value);
};

const openPortfolioPage = async () => {
  await AppLauncher.openUrl({ url: 'com.getjigra.myapp://page?id=portfolio' });
};
```
...
[`Info.plist`](https://jigrajs.web.app/docs/ios/configuration#configuring-infoplist).
