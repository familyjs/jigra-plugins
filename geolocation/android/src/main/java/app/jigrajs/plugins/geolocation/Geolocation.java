...
public class Geolocation {
  ...
  public void sendLocation(
    ...
    if (resultCode == ConnectionResult.SUCCESS) {
      ...
      if (this.isLocationServicesEnabled()) {
        ...
        int lowPriority = networkEnabled
  ...
  public Location getLastLocation(int maximumAge) {
    ...
    for (String provider : lm.getAllProviders()) {
      ...
      if (tmpLoc != null) {
        long locationAge =
          SystemClock.elapsedRealtimeNanos() - tmpLoc.getElapsedRealtimeNanos();
        long maximumAgeNanoSec = maximumAge * 1000000L;
        if (
          locationAge <= maximumAgeNanoSec &&
          (
            lastLoc == null ||
            lastLoc.getElapsedRealtimeNanos() > tmpLoc.getElapsedRealtimeNanos()
          )
        ) {
          lastLoc = tmpLoc;
        }
