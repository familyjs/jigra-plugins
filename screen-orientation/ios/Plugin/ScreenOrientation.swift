...
public class ScreenOrientation: NSObject {
    ...
    private var jigViewController: JIGBridgeViewController?

    public func setJigraViewController(_ viewController: JIGBridgeViewController) {
        self.jigViewController = viewController
    ...
    public func lock(_ orientationType: String, completion: @escaping (Error?) -> Void) {
        DispatchQueue.main.async {
            ...
            self.jigViewController?.supportedOrientations = [orientation]
    ...
    public func unlock(completion: @escaping (Error?) -> Void) {
        DispatchQueue.main.async {
            self.jigViewController?.supportedOrientations = self.supportedOrientations
            if #available(iOS 16.0, *) {
                if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene {
                    windowScene.keyWindow?.rootViewController?.setNeedsUpdateOfSupportedInterfaceOrientations()
