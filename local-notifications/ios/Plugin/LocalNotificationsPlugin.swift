...
public class LocalNotificationsPlugin: JIGPlugin {
     ...
     */
    func handleScheduledNotification(_ call: JIGPluginCall, _ schedule: JSObject) throws -> UNNotificationTrigger? {
        var at: Date?
    ...
    func makeActionTypes(_ actionTypes: [JSObject]) {
        ...
        for type in actionTypes {
            guard let id = type["id"] as? String else {
                JIGLog.print("⚡️ ", self.pluginId, "-", "Action type must have an id field")
                continue
            }
    ...
    func makeActions(_ actions: [JSObject]) -> [UNNotificationAction] {
        ...
        for action in actions {
            guard let id = action["id"] as? String else {
                JIGLog.print("⚡️ ", self.pluginId, "-", "Action must have an id field")
                continue
            }
