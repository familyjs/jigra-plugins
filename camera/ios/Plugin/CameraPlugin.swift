...
public class CameraPlugin: JIGPlugin {
    private var call: JIGPluginCall?
    ...
    @objc func pickLimitedLibraryPhotos(_ call: JIGPluginCall) {
        if #available(iOS 14, *) {
    ...
    @objc func getLimitedLibraryPhotos(_ call: JIGPluginCall) {
        if #available(iOS 14, *) {
            PHPhotoLibrary.requestAuthorization(for: .readWrite) { (granted) in
                if granted == .limited {
    ...
    @objc func getPhoto(_ call: JIGPluginCall) {
        ...
        if let missingUsageDescription = checkUsageDescriptions() {
            JIGLog.print("⚡️ ", self.pluginId, "-", missingUsageDescription)
    ...
    @objc func pickImages(_ call: JIGPluginCall) {
        self.multiple = true
        self.call = call
        self.settings = cameraSettings(from: call)
        DispatchQueue.main.async {
    ...
    private func cameraSettings(from call: JIGPluginCall) -> CameraSettings {
...
extension CameraPlugin: PHPickerViewControllerDelegate {
    ...
    private func fetchProcessedImages(from pickerResultArray: [PHPickerResult], accumulating: [ProcessedImage] = [],
                                      _ completionHandler: @escaping ([ProcessedImage]?) -> Void) {
...
private extension CameraPlugin {
    ...
    func showCamera() {
        ...
        if (bridge?.isSimEnvironment ?? false) || !UIImagePickerController.isSourceTypeAvailable(UIImagePickerController.SourceType.camera) {
            JIGLog.print("⚡️ ", self.pluginId, "-", "Camera not available in simulator")
