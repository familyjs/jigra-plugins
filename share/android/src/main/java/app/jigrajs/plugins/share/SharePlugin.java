package app.jigrajs.plugins.share;
...
public class SharePlugin extends Plugin {
      ...
      new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
          if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            ...
            chosenComponent =
              getParcelableExtraLegacy(intent, Intent.EXTRA_CHOSEN_COMPONENT);
          }
        }
      };
  ...
  private void activityResult(PluginCall call, ActivityResult result) {
    if (result.getResultCode() == Activity.RESULT_CANCELED && !stopped) {
      ...
      callResult.put(
        "activityType",
        chosenComponent != null ? chosenComponent.getPackageName() : ""
      );
  ...
  public void share(PluginCall call) {
    if (!isPresenting) {
      ...
      if (
        text == null && url == null && (files == null || files.length() == 0)
      ) {
